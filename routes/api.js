...
router.post('/generate-convention/:id', function(req, res) {
  ...
  execFile('python3', ['/home/debian/fill_convention2.py', JSON.stringify(data)], { timeout: 90000 }, function(err, stdout, stderr) {
    ...
    transporter.sendMail({
      ...
      subject: (pcEnabled ? 'Convention de formation - Parcours CAJA - ' : 'Convention de formation - ') + c.name + ' - Linguaid France',
      /* PARCOURS_CAJA_20260911: both programmes travel with the signing e-mail */
      attachments: pcEnabled ? [path.join(__dirname, '../data/programmes/' + c.id + '.pdf'), path.join(__dirname, '../data/programmes/' + c.id + '_m2.pdf')].filter(function(pp){ return fs.existsSync(pp); }).map(function(pp, i){ return { filename: 'programme_module' + (i + 1) + '_' + (c.name || 'candidat').replace(/\s+/g, '_') + '.pdf', path: pp }; }) : [],
      html: '<div style="font-family:Arial,sans-serif;font-size:14px;color:#222;line-height:1.6">' + convGreeting + (pcEnabled ? '<ul><li><strong>Parcours CAJA en deux modules</strong> \u2013 convention globale sur ' + pcCalc.maxMonths + ' mois</li><li><strong>Module 1\u00a0:</strong> ' + pcCalc.modules[0].title + ' \u2013 ' + pcCalc.modules[0].totalHours + 'h \u2013 ' + parcoursLib.fmtEur(pcCalc.modules[0].price) + '</li><li><strong>Module 2\u00a0:</strong> ' + pcCalc.modules[1].title + ' \u2013 ' + pcCalc.modules[1].totalHours + 'h \u2013 ' + parcoursLib.fmtEur(pcCalc.modules[1].price) + '</li><li><strong>Total du parcours\u00a0:</strong> ' + parcoursLib.fmtEur(pcCalc.totals.price) + ', dont ' + parcoursLib.fmtEur(pcCalc.totals.cpf) + ' financ\u00e9s par le CPF</li></ul>' : '') + '<ul><li><strong>Formation :</strong> ' + convTitle + '</li><li><strong>Durée :</strong> ' + (od.totalHours || '—') + 'h</li><li><strong>Dates :</strong> du ' + (dateStart ? fmtDateFr(dateStart) : '—') + ' au ' + (dateEnd ? fmtDateFr(dateEnd) : '—') + '</li><li><strong>Tarif HT :</strong> ' + (cd.price || '—') + ' €</li>' + (data.instalments > 1 ? '<li><strong>R\u00e8glement :</strong> ' + paymentTermsFr(data.price, data.instalments) + '</li>' : '') + '</ul><p>Pour valider cette convention, veuillez cliquer sur le lien ci-dessous et signer électroniquement :</p><p><a href="' + signingUrl + '" style="background:#1F4E79;color:white;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:bold">Signer la convention</a></p><p>Ou copiez ce lien dans votre navigateur : <a href="' + signingUrl + '">' + signingUrl + '</a></p><p>N’hésitez pas à nous contacter pour toute question.</p><p>Bien cordialement,</p><img src="https://eval.linguaid.net/signature_joss.png" style="max-width:400px"></div>'
    }, function(mailErr) {
...
router.post('/typeform-webhook', function(req, res) {
  ...
  if (resolvedCount === 0) {
    ...
    try {
      ...
      require('../lib/mailer').createTransport().sendMail({
...
router.get('/download-proposition/:id', function(req, res) {
  ...
  if (!require('fs').existsSync(propPdf)) return res.status(404).json({ error: 'Proposition PDF not found - generate first' });
...
var CONVOC_TRAINERS = {"anna": {"name": "Anna Malzy", "email": "ajmalzy@gmail.com", "tel": "07 70 00 02 06"}, "hannah": {"name": "Hannah Durrant", "email": "coursdanglais24@gmail.com", "tel": "07 86 87 11 72"}, "leone": {"name": "Leone Crinnion", "email": "crinnion@gmail.com", "tel": "07 67 31 66 91"}, "stephanie": {"name": "Stephanie Cooper-Slockyj", "email": "slockyj@gmail.com", "tel": "06 62 12 04 29"}, "natasha": {"name": "Natasha Costello", "email": "nclegalenglish@gmail.com", "tel": "06 31 61 72 00"}, "louisek": {"name": "Louise Kulbicki", "email": "lbkulbicki@gmail.com", "tel": "0039 320 7958909"}, "louiseg": {"name": "Louise Garavaglia", "email": "louise.garavaglia@gmail.com", "tel": "06 08 96 21 00"}, "lynsey": {"name": "Lynsey Redfern", "email": "redfern.lynsey@gmail.com", "tel": "06 33 39 03 44"}, "joss": {"name": "Joss Frimond", "email": "jossfrimond@hotmail.com", "tel": "06 84 49 23 32"}};
...
router.post('/send-convocation/:id', function(req, res) {
  try {
    ...
    var civility = cd.civility || '';
    var lastName = (c.name || '').split(' ').slice(-1)[0];
    var greeting = civility ? civility + ' ' + lastName : lastName;

    var SEP = '\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500';

    var volet2Section = '';
    if (hasYYKE) {
      volet2Section = '<p><strong>' + SEP + '<br>VOLET 2 \u2014 MODULE E-LEARNING (' + homeworkHours + ' heures)<br>' + SEP + '</strong></p>'
        + '<p>Module : <em>\u00ab Yes, you Ken English \u00bb</em> \u2013 anglais professionnel en contexte juridique, en ligne</p>'
        + '<p>Modalit\u00e9 : acc\u00e8s \u00e0 distance, en autonomie accompagn\u00e9e</p>'
        + '<p>Vos codes d\u2019acc\u00e8s \u00e0 la plateforme vous seront communiqu\u00e9s s\u00e9par\u00e9ment dans les prochains jours.</p>';
    } else if (homeworkHours > 0) {
      volet2Section = '<p><strong>' + SEP + '<br>VOLET 2 \u2014 MODULE TRAVAUX PERSONNELS (' + homeworkHours + ' heures)<br>' + SEP + '</strong></p>'
        + '<p>Module : <em>\u00ab Travaux personnels \u00bb</em> \u2013 travaux pr\u00e9par\u00e9s par votre coach en anglais professionnel</p>'
        + '<p>Modalit\u00e9 : les travaux seront envoy\u00e9s sur votre bo\u00eete mail asynchrone avec vos cours de visio</p>';
    }
    ...
    if (isCPF) {
      var certName = cpfType === 'CAJA'
        ...
        : trainingTitle;
      certSection = '<p><strong>' + SEP + '<br>\u00c9TAPE FINALE \u2014 CERTIFICATION<br>' + SEP + '</strong></p>'
        + '<p>\u00c0 l\u2019issue des deux volets de formation, vous passerez la certification :</p>'
        + '<p><strong>' + certName + '</strong></p>'
        + '<p>Nous vous accompagnerons dans les d\u00e9marches d\u2019inscription \u00e0 l\u2019examen et vous communiquerons les modalit\u00e9s de passage en temps utile.</p>';
    }

    var sigPath = path2.join(__dirname, '../views/signature_catherine.png');
    var sigCid = 'signature_catherine';
    var sigHtml = fs2.existsSync(sigPath)
      ? '<img src="cid:' + sigCid + '" alt="Signature Catherine" style="max-width:400px;display:block;margin-top:8px">'
      : '<p><strong>Catherine Frimond-Laubi\u00e8s</strong><br>Responsable suivi<br>+33 699 10 68 54<br>cfr@linguaid.net</p>';

    // Build documents list
    var docsList = '<ul><li>Programme de formation</li><li>Livret d\u2019accueil (dont r\u00e8glement int\u00e9rieur)</li>';
    if (isCPF && (cpfType === 'E360' || cpfType === 'E360_LEGAL')) {
      docsList += '<li>Kit bienvenue English 360</li>';
    }
    if (isCPF && cpfType === 'CAJA') {
      docsList += '<li>Aper\u00e7u de la certification CAJA</li>';
    }
    docsList += '</ul>';
    ...
    var html = '<div style="font-family:Arial,sans-serif;font-size:14px;color:#222;line-height:1.7;max-width:680px">'
      ...
      + '<p>Nous avons le plaisir de vous confirmer votre nouvelle formation et vous adressons la pr\u00e9sente convocation.</p>'
      + '<p><strong>' + SEP + '<br>VOTRE PARCOURS DE FORMATION<br>' + SEP + '</strong></p>'
      + '<p>' + (isCPF ? 'Niveau vis\u00e9 : ' : 'Intitul\u00e9 de la formation : ') + trainingTitle + '<br>'
      + 'Organisme de formation : Linguaid France SAS<br>'
      + 'Dur\u00e9e totale : ' + totalHours + ' heures</p>'
      + '<p><strong>' + SEP + '<br>VOLET 1 \u2014 COURS INDIVIDUELS EN VISIO (' + coachingHours + ' heures)<br>' + SEP + '</strong></p>'
      + '<p>Formateur : ' + trainer.name + '<br>'
      + 'Contact : ' + trainer.email + (trainer.tel ? ' \u2013 ' + trainer.tel : '') + '<br>'
      + 'Modalit\u00e9 : cours particuliers par visioconf\u00e9rence (Zoom ou Teams)</p>'
      + (firstSession ? '<p>Premi\u00e8re s\u00e9ance : <strong>' + firstSession + '</strong></p>' : '')
      + '<p>Les s\u00e9ances suivantes seront planifi\u00e9es directement avec votre formateur selon vos disponibilit\u00e9s respectives.</p>'
      + '<p><em>Politique d\u2019annulation (volet 1 uniquement) : toute annulation par le stagiaire intervenant moins de 48 heures avant le d\u00e9but d\u2019une s\u00e9ance sera comptabilis\u00e9e comme une heure effectivement r\u00e9alis\u00e9e.</em></p>'
      + volet2Section
      + certSection
      + '<p><strong>' + SEP + '<br>DOCUMENTS JOINTS<br>' + SEP + '</strong></p>'
      + docsList
      + '<p>Nous restons \u00e0 votre disposition pour toute question relative \u00e0 votre parcours. Dans le cadre de notre d\u00e9marche qualit\u00e9 (certification Qualiopi), nous serons \u00e9galement en contact avec vous en cours et en fin de formation afin de nous assurer que celle-ci correspond \u00e0 vos attentes et objectifs.</p>'
      + '<p>Have a great training !</p>'
      + '<p>Bien cordialement,</p>'
      + sigHtml
      + '</div>';
    ...
    if (isCPF && (cpfType === 'E360' || cpfType === 'E360_LEGAL')) {
      var kitPath = path2.join(__dirname, '../views/welcome_kit_e360.pdf');
      if (fs2.existsSync(kitPath)) {
        attachments.push({ filename: 'welcome_kit_english360.pdf', path: kitPath });
      }
    }
    if (isCPF && cpfType === 'CAJA') {
      var cajaPath = path2.join(__dirname, '../views/apercu_caja.pdf');
      if (fs2.existsSync(cajaPath)) {
        attachments.push({ filename: 'apercu_certification_caja.pdf', path: cajaPath });
      }
    }
    if (fs2.existsSync(sigPath)) {
    ...
    transporter.sendMail({
      ...
      console.log('Convocation sent for ' + c.name);
    ...
    console.error('send-convocation caught error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/send-attestation-signing/:id', function(req, res) {
  try {
    ...
    var transporter2 = require('../lib/mailer').createTransport();
...
function approveReminders(req, res, match) {
  try {
    ...
    var nodemailer = require('nodemailer');
    var t = require('../lib/mailer').createTransport();
    var sent = [], skipped = [];
...
/* devis-opco (2026-08-10): devis client conforme OPCO (financement entreprise / OPCO).
   Utilise views/DEVIS_OPCO_TEMPLATE.docx (en-tete + pied de page officiels Linguaid)
   via /home/debian/fill_devis_opco.py -> data/devis_opco/devis_opco_{id}.pdf|docx */
router.post('/generate-devis-opco/:id', function(req, res) {
  var fs2 = require('fs');
  var path2 = require('path');
  var execFile2 = require('child_process').execFile;
  var candidates = getCandidates();
  ...
  var c = candidates[idx];
  var b = req.body || {};
  var clientBlock = String(b.clientBlock || '').trim();
  if (!clientBlock) return res.status(400).json({ error: 'Coordonnees entreprise requises' });
  var hours = parseFloat(String(b.hours || '').replace(',', '.'));
  var rate = parseFloat(String(b.rate || '').replace(',', '.'));
  if (!(hours > 0) || !(rate > 0)) return res.status(400).json({ error: 'Heures et tarif invalides' });
  var trav = parseFloat(String(b.travauxMontant || '0').replace(',', '.')) || 0;
  var totalNum = hours * rate + trav;
  function eur(n) {
  ...
  var hoursStr = String(hours).replace('.', ',');
  var outDir = path2.join(__dirname, '../data/devis_opco');
  var args = {
    devisNumber: devisNumber,
    dateEmission: today,
    dateValidite: String(b.dateValidite || '').trim() || ('31 d\u00e9cembre ' + yr),
    clientBlock: clientBlock,
    clientName: clientBlock.split('\n')[0].trim(),
    intitule: String(b.intitule || '').trim() || ('Formation en anglais ' + (isLegal ? 'juridique' : 'professionnel') + ' \u2014 Action individuelle'),
    stagiaire: String(b.stagiaire || '').trim() || (c.name || ''),
    ...
    designation: String(b.designation || '').trim() || ('Coaching individuel en anglais ' + (isLegal ? 'juridique' : 'professionnel') + ' \u2014 s\u00e9ances en visioconf\u00e9rence, contenus personnalis\u00e9s'),
    qte: hoursStr + ' h',
    pu: eur(rate),
    montant: eur(hours * rate),
    travauxLabel: trav > 0 ? (String(b.travauxLabel || '').trim() || 'Travaux d\u2019intersession \u2014 pr\u00e9paration et corrections personnalis\u00e9es (forfait)') : '',
    travauxQte: trav > 0 ? '\u2014' : '',
    travauxPu: trav > 0 ? '\u2014' : '',
    travauxMontant: trav > 0 ? eur(trav) : '',
    total: eur(totalNum),
    outDir: outDir,
    id: c.id
  };
  if (!fs2.existsSync(outDir)) fs2.mkdirSync(outDir, { recursive: true });
  execFile2('python3', ['/home/debian/fill_devis_opco.py', JSON.stringify(args)], { timeout: 120000 }, function(err, stdout, stderr) {
    if (err) { console.error('fill_devis_opco error:', stderr, stdout); return res.status(500).json({ error: 'Devis generation failed' }); }
    var result;
    try { result = JSON.parse(stdout.trim()); } catch (e) { return res.status(500).json({ error: 'Invalid fill_devis_opco output' }); }
    if (!result.success) { console.error('fill_devis_opco:', result); return res.status(500).json({ error: result.error }); }
    var cands2 = getCandidates();
    var i2 = cands2.findIndex(function(x){ return x.id === req.params.id; });
    if (i2 !== -1) {
      cands2[i2].devisOpco = {
        number: devisNumber,
        total: args.total,
    ...
    res.json({ success: true, total: args.total });
  });
});

router.get('/download-devis-opco/:id', function(req, res) {
  var pp = require('path');
  var pdf = pp.join(__dirname, '../data/devis_opco/devis_opco_' + req.params.id + '.pdf');
  if (!require('fs').existsSync(pdf)) return res.status(404).json({ error: 'Devis PDF not found - generate first' });
  var cands = getCandidates();
  var c = cands.find(function(x){ return x.id === req.params.id; });
  var name = (c && c.name) ? c.name.replace(/\s+/g, '_') : req.params.id;
  res.setHeader('Cache-Control', 'no-store');
  res.setHeader('Content-Type', 'application/pdf');
  res.setHeader('Content-Disposition', 'attachment; filename="devis_' + name + '.pdf"');
  res.sendFile(pdf);
});

router.get('/download-devis-opco-docx/:id', function(req, res) {
  var pp = require('path');
  var f = pp.join(__dirname, '../data/devis_opco/devis_opco_' + req.params.id + '.docx');
  if (!require('fs').existsSync(f)) return res.status(404).json({ error: 'Devis DOCX not found - generate first' });
  var cands = getCandidates();
  var c = cands.find(function(x){ return x.id === req.params.id; });
  var name = (c && c.name) ? c.name.replace(/\s+/g, '_') : req.params.id;
  res.setHeader('Cache-Control', 'no-store');
