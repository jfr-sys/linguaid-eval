...
/* TIERS_PREFILL_20260908: per-company memory of convention details
   (siret, address, signatory, signatoryEmail, signatoryCivility,
   signatoryPrenom, signatoryNom). Stored as entry.info in companies.json.
   Only non-empty values are written; Particulier/blank names are ignored. */
var INFO_FIELDS = ['siret', 'address', 'signatory', 'signatoryEmail', 'signatoryCivility', 'signatoryPrenom', 'signatoryNom'];
function findEntryKey(reg, raw) {
  var key = normKey(raw);
  if (!key || key === 'particulier') return null;
  if (reg.companies[key]) return key;
  var keys = Object.keys(reg.companies);
  for (var i = 0; i < keys.length; i++) {
    var aliases = reg.companies[keys[i]].aliases || [];
    for (var j = 0; j < aliases.length; j++) {
      if (normKey(aliases[j]) === key) return keys[i];
    }
  }
  return null;
}
function getCompanyInfo(raw) {
  var reg = loadRegistry();
  var k = findEntryKey(reg, raw);
  if (!k) return {};
  var info = reg.companies[k].info || {};
  var out = {};
  INFO_FIELDS.forEach(function(f) { if (info[f]) out[f] = info[f]; });
  return out;
}
function updateCompanyInfo(raw, fields) {
  var reg = loadRegistry();
  var k = findEntryKey(reg, raw);
  if (!k) return false;
  var info = reg.companies[k].info || {};
  ...
  INFO_FIELDS.forEach(function(f) {
    var v = String((fields || {})[f] || '').trim();
    if (v && info[f] !== v) { info[f] = v; changed = true; }
  });
  if (changed) { reg.companies[k].info = info; saveRegistry(reg); }
  return changed;
}

module.exports = { canonicalCompany: canonicalCompany, listCompanies: listCompanies, normKey: normKey,
  getCompanyInfo: getCompanyInfo, updateCompanyInfo: updateCompanyInfo };
